...
const ReportVacanncy = ({ month, year }) => {
  ...
  const reportStructure = [
    ...
    { postName: "S.S.T (Science + Arts)", bps: "16 & 17", isTeachingStaff: true },
  ...
  const fetchData = async () => {
    try {
      ...
      const [
        ...
        supabase.from("School").select("NoOfSactions").order("SchoolID", { ascending: true }),
        supabase.from("Teacher").select("Post"),
        supabase.from("staff").select("designation")
      ]);

      // Error handling
      if (sanctionedError) throw new Error(`Sanctioned data: ${sanctionedError.message}`);
      if (teachersError) throw new Error(`Teachers data: ${teachersError.message}`);
      if (staffError) throw new Error(`Staff data: ${staffError.message}`);

      // Count working teaching staff
      const teachingWorkingCounts = {};
      Object.entries(teachingPostMappings).forEach(([reportPost, {matches, exact}]) => {
        teachingWorkingCounts[reportPost] = teachers.filter(teacher => {
          const post = teacher.Post?.toLowerCase() || '';
          return matches.some(match => 
            exact ? post === match : post.includes(match)
      ...
      // Count non-teaching staff
      const nonTeachingWorkingCounts = {};
      const nonTeachingPosts = reportStructure.filter(item => !item.isTeachingStaff);
      
      nonTeachingPosts.forEach(item => {
        const designation = postNameMapping[item.postName]?.toLowerCase();
        nonTeachingWorkingCounts[item.postName] = staff.filter(s => 
          s.designation?.toLowerCase() === designation
        ).length;
      });

      // Principal counts
      const principalCount = teachers.filter(t => 
        t.Post?.toLowerCase() === "principal"
      ).length;
      
      const vicePrincipalCount = teachers.filter(t => 
        t.Post?.toLowerCase() === "vice principal"
      ).length;

      // Generate report data
      const reportData = reportStructure.map(item => {
        const supabasePostName = postNameMapping[item.postName];
        const sanctioned = sanctionedData?.[0]?.NoOfSactions?.[supabasePostName]?.Sanctioned || 0;
  ...
  const calculateSummaryData = (data) => {
    const teachingStaff = { sanctioned: 0, working: 0, vacant: 0, excess: 0 };
    const nonTeachingStaff = { sanctioned: 0, working: 0, vacant: 0, excess: 0 };
    const total = { sanctioned: 0, working: 0, vacant: 0, excess: 0 };
  ...
  const generatePDFDocument = async () => {
    try {
      ...
      pdf.setFontSize(16);
      pdf.text(
        `Vacancy Report - ${new Date(year, month - 1).toLocaleString("default", { month: "long" })} ${year}`,
        pdf.internal.pageSize.getWidth() / 2,
        15,
        { align: "center" }
      );
      ...
      autoTable(pdf, {
        head: [summaryColumns.map((col) => col.header)],
        body: summaryRows.map((row) => summaryColumns.map((col) => row[col.dataKey])),
        startY: summaryY,
        theme: "grid",
        headStyles: {
          fillColor: [52, 152, 219],
