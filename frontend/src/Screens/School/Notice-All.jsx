...
import SearchIcon from "@mui/icons-material/Search";
import { supabase } from "../../../supabase-client"; // adjust if needed
...
export default function AllNotices() {
  ...
  // Fetch notices with filter for Admin or same-school-created
  useEffect(() => {
    if (!schoolId) return;
    (async () => {
      setLoading(true);
      try {
        ...
        const filteredNotices = data.filter(
          (n) =>
            n.CreatedType === "Admin" ||
            (n.CreatedType === "School" && n.CreatedBy === schoolId)
        );
