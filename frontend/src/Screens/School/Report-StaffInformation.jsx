...
const ReportStaffInformation = ({ month, year, SchoolID, SchoolName }) => {
  ...
  const fetchTeachersData = async () => {
    ...
    try {
      const { data: teachers, error } = await supabase
        ...
        .neq("EmployeeType", "Teacher");
      if (error) throw error;

      const { data: staff, stafferror } = await supabase
        .from("staff")
        .select("*")
        .order("id", { ascending: true });
      // .neq("EmployeeType", "Teacher");`
      if (stafferror) throw error;
      ...
      const formattedTotalData = staff.map((staff, index) => ({
        srNo: formattedData.length + index + 1,
        id: staff.employee_id,
        nameWithPercentage: `${staff.full_name} ${
          staff.father_name ? `(${staff.father_name})` : ""
        }`,
        designation: staff.designation || "N/A",
        eps: staff.BPS || "N/A",
        dateOfBirth: staff.date_of_birth || "N/A",
        domicile: staff.Domicile || "N/A",
        qualification: staff.Qualification || "N/A",
        dateJoiningDept: staff.joining_date || "N/A",
        dateJoiningSchool: staff.joining_date || "N/A",
        regular: staff.employee_type === "Regular" ? "Yes" : "No",
        contract: staff.employee_type === "Contract" ? "Yes" : "No",
        deputation: staff.employee_type === "Deputation" ? "Yes" : "No",
      }));

      // Combine both datasets
      const reportData = [...formattedData, ...formattedTotalData];
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const fileName = `Monthly Staff Profile Report- ${SchoolID} -${month}- ${year}.pdf`;
      pdf.save(fileName);
  ...
  const savePDFToDatabase = async () => {
    ...
    try {
      const { data: existingReport, error: fetchError } = await supabase
        .from("SavedReports")
        .select("*")
        .eq("ReportName", "Monthly Staff Profile Report")
        .eq("Month", month)
        .eq("Year", year)
      ...
      const fileName = `Monthly_staff_profile_${SchoolID}_${month}_${year}.pdf`;
      ...
      const { error: dbError } = await supabase.from("SavedReports").insert({
        ...
        Year: parseInt(year),
        ReportName: "Monthly Staff Profile Report",
        FileName: fileName,
        created_at: new Date().toISOString(),
        FilePath: publicUrl,
        ReportType: "monthly_staff_profile",
