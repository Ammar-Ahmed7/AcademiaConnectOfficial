...
const SchoolManagement = () => {
  ...
  const generateSchoolId = async (schoolFor) => {
    try {
      ...
      if (data && data.length > 0) {
        const lastId = data[0].SchoolID
        const lastNumber = Number.parseInt(lastId.split("-")[2]) || 0
        nextNumber = lastNumber + 1
      }
  ...
  // Check if email already exists
  const checkEmailExists = async (email) => {
    const { data, error } = await supabase.from("School").select("Email").eq("Email", email).maybeSingle()
    if (error) {
      console.error("Error checking email:", error)
      return false
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!/^\d{9,12}$/.test(trimmedData.phoneNumber)) {
      setAlert({
        open: true,
        message: "Please enter a valid phone number (9-12 digits, only numbers allowed)",
        severity: "error",
      })
      setLoading(false)
      return
    ...
    if (
      ...
      setAlert({
        open: true,
        message: "Both 'Recognized By Board' and 'Attestation ID' are required together!",
        severity: "error",
      })
      setLoading(false)
      return
            ...
            <form onSubmit={handleFormSubmit}>
              <Grid container spacing={isMobile ? 1.5 : 2}>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  >
                    <InputLabel>School For</InputLabel>
                    <Select label="School For" name="schoolfor" value={formData.schoolfor} onChange={handleInputChange}>
                      <MenuItem value="Girls">Girls</MenuItem>
                      <MenuItem value="Boys">Boys</MenuItem>
                    </Select>
                  </FormControl>
                ...
                <Grid item xs={12}>
                  ...
                  <Grid container spacing={isMobile ? 0.5 : 1} alignItems="center">
                    {["library", "sports", "computerLab", "scienceLab"].map((facility) => (
                      <Grid item key={facility} xs={12} sm={6} md={3}>
                        <FormControlLabel
                          control={
                            <Checkbox
                              checked={formData[facility]}
                ...
                <Grid item xs={12}>
                  ...
                  >
                    {loading ? <CircularProgress size={isMobile ? 20 : 24} /> : "Add School"}
                  </Button>
                </Grid>
              </Grid>
            </form>
      ...
      >
        ...
        >
          ...
          <Button
            ...
            sx={{ fontSize: isMobile ? "0.75rem" : "0.875rem" }}
          >
            {loading ? <CircularProgress size={isMobile ? 16 : 24} /> : "Confirm"}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
